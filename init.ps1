...
Add-HostsEntry "cm.basiccompany.localhost"
if ($Topology -ne "xp0") {
  Add-HostsEntry "cd.basiccompany.localhost"
}
Add-HostsEntry "id.basiccompany.localhost"
Add-HostsEntry "www.basiccompany.localhost"
Add-HostsEntry "hrz.basiccompany.localhost"
...
if ($InitEnv) {
    ...
    # CM_HOST
    Set-EnvFileVariable "CM_HOST" -Value "cm.basiccompany.localhost"

    if ($Topology -ne "xp0") {
      # CD_HOST
      Set-EnvFileVariable "CD_HOST" -Value "cd.basiccompany.localhost"
    }
	
	# HRZ_HOST
    Set-EnvFileVariable "HRZ_HOST" -Value "hrz.basiccompany.localhost"

    # ID_HOST
    Set-EnvFileVariable "ID_HOST" -Value "id.basiccompany.localhost"

    # RENDERING_HOST
    Set-EnvFileVariable "RENDERING_HOST" -Value "www.basiccompany.localhost"
	...
	# SITECORE_LICENSE
	Set-EnvFileVariable "SITECORE_LICENSE" -Value (ConvertTo-CompressedBase64String -Path "$($LicenseXmlPath)\license.xml")
...
Write-Host "Installing dotent tool to enable Sitecore CLI commands" -ForegroundColor Green
dotnet tool restore

# Install npm dependencies in the rendering folder
Write-Host "Installing npm dependencies in src\rendering..." -ForegroundColor Green
$currentLocation = Get-Location
try {
    Set-Location "src\rendering"
    npm install
    if ($LASTEXITCODE -ne 0) {
        Write-Error "npm install failed in src\rendering folder."
    }
    Write-Host "npm install completed successfully." -ForegroundColor Green
} catch {
    Write-Error "Failed to run npm install in src\rendering folder: $_"
} finally {
...
Write-Host
Write-Host "Setting NODE_EXTRA_CA_CERTS environment variable..." -ForegroundColor Green
$currentLocation = Get-Location
try {
    Set-Location "src\rendering"
    setx NODE_EXTRA_CA_CERTS $caRoot
    Write-Host "NODE_EXTRA_CA_CERTS environment variable has been set." -ForegroundColor Green
    Write-Host "You will need to restart your terminal or VS Code for it to take effect." -ForegroundColor Cyan
} catch {
    Write-Error "Failed to set NODE_EXTRA_CA_CERTS environment variable: $_"
} finally {
    Set-Location $currentLocation
}
