...
# copy any library files that may be referenced by projects
# COPY lib/ /lib/

SHELL ["powershell", "-Command", "$ErrorActionPreference = 'Stop'; $ProgressPreference = 'SilentlyContinue';"]
RUN Invoke-Expression 'robocopy C:/temp C:/nuget/src /s /ndl /njh /njs *.csproj *.scproj packages.config'
RUN Invoke-Expression 'robocopy C:/lib C:/nuget/lib /s /ndl /njh /njs *.*'
...
RUN msbuild .\src\platform\_Deployment\website\BasicCompany.Deployment.csproj /p:Configuration=$env:BUILD_CONFIGURATION /m /p:DeployOnBuild=true /p:PublishProfile=Local
